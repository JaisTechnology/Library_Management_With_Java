...
public class Login extends JFrame implements ActionListener
{
		...
		{ 
			...
			if(user.equals("Admin") && pass.equals("TYCM5"))
			{
...
class LibraryHome extends JFrame implements ActionListener
{
		...
		LibraryHome()
		{
			Container c=getContentPane();
			c.setLayout(null);
			ImageIcon iix=new ImageIcon("L1.jpg");
			ImageIcon iiy=new ImageIcon("Head.jpeg");
...
class NewStudent extends JFrame implements ActionListener
{
		...
		public void actionPerformed(ActionEvent ae)
		{
			try
			{
				...
				//ResultSet Rs=stmt.executeQuery("create table Student(Student_Name varchar(30),rollno number(3),branch varchar(5),mobileno number(10))");
				if(ae.getSource()==btn1)
				{
					int cnt=stmt.executeUpdate("insert into Student values('"+tf1.getText()+"',"+Integer.parseInt(tf2.getText())+",'"+tf3.getText()+"',"+Long.parseLong(tf4.getText())+")");
				...
				{
					int cnt=stmt.executeUpdate("update Student set Student_Name='"+tf1.getText()+"',rollno="+Integer.parseInt(tf2.getText())+", branch='"+tf3.getText()+"',mobileno="+Integer.parseInt(tf4.getText())+" where rollno="+Integer.parseInt(tf2.getText()));
				...
				while(rs.next())
				{
					System.out.println(rs.getString(1)+" "+rs.getInt(2)+" "+rs.getString(3)+" "+rs.getLong(4));
				}
				con.close();
			}
			catch(Exception e)
...
class NewBook extends JFrame implements ActionListener
{
		...
		public void actionPerformed(ActionEvent ae)
		{
			try
			{
				...
				System.out.println("Database Connection Successfull!!!");
				Statement stmt=con.createStatement();
				//ResultSet Rj=stmt.executeQuery("create table Book(bname varchar(30),bid number(4),publisher varchar(10),price number(4))");
				if(ae.getSource()==btn1)
				{
...
class IssueBook extends JFrame implements ActionListener
{
		...
		public void actionPerformed(ActionEvent ae)
		{
			try
			{
				...
				System.out.println("Database Connection Successfull!!!");
				Statement stmt=con.createStatement();
				//ResultSet Rt=stmt.executeQuery("create table IssueBook(bid number(3),sid number(3),idate varchar(15),ddate varchar(15))");
